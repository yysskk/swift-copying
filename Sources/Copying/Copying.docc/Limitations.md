...
- **A property with an `init` accessor** (SE-0400). Such a property is computed, yet it stands in for the storage it initializes in the memberwise initializer, so a copy would have to pass the property and leave that storage out. Only the accessor's `@storageRestrictions` says which storage that is, and routing a copy through the accessor is not something the macro can check, so a type that declares one needs a `copying` written by hand. (Macros that generate `init` accessors, such as `@Observable`, are unaffected: each macro sees the declaration as written, not what another expands it into.)
