...
- A warning on an initializer that takes the copied properties but is failable, throwing, or `async`, since `copying` calls it as a plain expression. This too previously surfaced only inside the generated code, as an unwrapping, `try`, or `await` error.
